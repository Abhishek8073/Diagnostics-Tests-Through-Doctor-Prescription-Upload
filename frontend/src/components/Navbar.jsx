...
import { FaUserCircle, FaSignOutAlt, FaHome, FaStethoscope } from "react-icons/fa";
...
export default function Navbar() {
  ...
  <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
    ...
    <a
      href={brand.href}
      className={`flex shrink-0 items-center gap-2 text-lg font-bold tracking-tight ${hasRoleGradient ? 'text-white' : 'text-slate-900'}`}
    >
      <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-brand-600 text-white shadow-sm"><FaStethoscope size={17} /></span>
    ...
    <div className="flex items-center gap-2 sm:gap-4">
      ...
      <Link to="/" title="Back to landing page" className={`flex h-9 w-9 items-center justify-center rounded-xl hover:-translate-y-0.5 ${hasRoleGradient ? 'bg-white/15 text-white hover:bg-white/25' : 'bg-brand-50 text-brand-700 hover:bg-brand-100'}`}>
        <FaHome size={16} />
      </Link>
      ...
      <button
        ...
        className={`flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium ${hasRoleGradient ? 'bg-white/15 text-white hover:bg-rose-400/25 hover:text-white' : 'bg-slate-100 text-slate-700 hover:bg-rose-50 hover:text-rose-600'}`}
      >
